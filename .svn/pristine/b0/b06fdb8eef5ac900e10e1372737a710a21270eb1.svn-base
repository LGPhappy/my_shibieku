...
import com.ocrlib.zdsb.auth.RecognizerAuth;
import com.ocrlib.zdsb.bean.DriverIDCard;
import com.ocrlib.zdsb.bean.SecondIDCard;
import com.ocrlib.zdsb.utils.Constant;
import com.ocrlib.zdsb.utils.FileUtils;
import com.ocrlib.zdsb.utils.Zzlog;
import com.ocrlib.zdsb.R;
...
public class IDCardRecognizeActivity extends Activity
    ...
    boolean isShowDialog = true; // default
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Zzlog.out(TAG, "isShowDialog : " + isShowDialog);
        ...
        if (!RecognizerAuth.check(intent)) {
            ...
            retIntent.putExtra("json", "");
            setResult(RESULT_OK, retIntent);
            finish();
        }
        savePic = intent.getBooleanExtra("savePic", true);
        Zzlog.out(TAG, "savePic = " + savePic);
        ...
        public void onServiceConnected(ComponentName name, IBinder service) {
            ...
            try {
                ...
                if (resultMessage.ReturnAuthority == 0 && resultMessage.ReturnInitIDCard == 0
                    ...
                    Zzlog.out(TAG, "recog Result String:" + recogResultString);
    ...
    protected void setRecognizeResult(String recogResult) {
        ...
        if (nMainIDX == 2 && secondIDCard != null) {
            intent.putExtra("SFZH", secondIDCard.getSfzh());
            intent.putExtra("json", gson.toJson(secondIDCard));
        }
        if (nMainIDX == 5 && driverIDCard != null) {
            intent.putExtra("SFZH", driverIDCard.getSfzh());
            intent.putExtra("json", gson.toJson(driverIDCard));
        }
    ...
    private void addHeadPic(Intent intent) {
         Bitmap bitmap = BitmapFactory.decodeFile(HeadJpgPath);
         String base64img = FileUtils.bitmapToBase64(bitmap);
